...
追加の[Babelの設定オプション](https://babeljs.io/docs/options)をPreactのViteプラグインに渡せます。Preactコンポーネントに適用されるBabelの変換をカスタマイズできます。

たとえば、次の設定はPreactコンポーネントを処理するときにBabelに`.babelrc`を読み込ませます。
